...
function validateA2UIComponents(input) {
	...
	components.forEach((comp, i) => {
		...
		if (isObject(comp)) {
			collectComponentRefEdges(comp, catalog && typeof type === "string" ? catalog.components[type] : void 0).forEach(({ path: refPath, ref }) => {
...
*/
function collectComponentRefEdges(comp, schema) {
	const edges = [];
	const pushSingle = (field, value) => {
		collectChildRefs(value).forEach((ref) => edges.push({
			path: field,
			ref
		}));
	};
	const pushList = (field, value) => {
		if (Array.isArray(value)) value.forEach((item, k) => collectChildRefs(item).forEach((ref) => edges.push({
			path: `${field}[${k}]`,
			ref
		})));
		else collectChildRefs(value).forEach((ref) => edges.push({
			path: field,
			ref
		}));
	};
	...
	if (isObject(props)) for (const [field, propSchema] of Object.entries(props)) {
		if (field === "child" || field === "children" || !isObject(propSchema)) continue;
		const fmt = propSchema.format;
		if (fmt === "componentRef") pushSingle(field, comp[field]);
		else if (fmt === "componentRefList") pushList(field, comp[field]);
			...
			if (isObject(itemProps) && Array.isArray(arrVal)) arrVal.forEach((item, k) => {
				if (!isObject(item)) return;
				for (const [sub, subSchema] of Object.entries(itemProps)) {
					if (!isObject(subSchema)) continue;
					if (subSchema.format === "componentRef") collectChildRefs(item[sub]).forEach((ref) => edges.push({
						path: `${field}[${k}].${sub}`,
						ref
					}));
					else if (subSchema.format === "componentRefList") {
						const subVal = item[sub];
						if (Array.isArray(subVal)) subVal.forEach((sv, j) => collectChildRefs(sv).forEach((ref) => edges.push({
							path: `${field}[${k}].${sub}[${j}]`,
							ref
						})));
						else collectChildRefs(subVal).forEach((ref) => edges.push({
							path: `${field}[${k}].${sub}`,
							ref
						}));
					}
...
function childAdjacency(components, catalog) {
	const adj = /* @__PURE__ */ new Map();
	for (const comp of components) if (isObject(comp) && typeof comp.id === "string") {
		const type = typeof comp.component === "string" ? comp.component : void 0;
		const schema = catalog && type ? catalog.components[type] : void 0;
		adj.set(comp.id, collectComponentRefEdges(comp, schema).map((e) => e.ref));
	}
	return adj;
}
/**
...
*/
function findChildCycles(components, catalog) {
...
*/
function splitA2UISchemaContext(context) {
	let schemaValue;
	const regular = [];
	for (const entry of context ?? []) if (entry?.description === A2UI_SCHEMA_CONTEXT_DESCRIPTION) schemaValue = entry?.value;
...
*/
function resolveA2UICatalog(state) {
	const agUi = state["ag-ui"] ?? {};
	const a2uiSchema = agUi.a2ui_schema;
	if (a2uiSchema) {
		let catalogId;
		try {
			const parsed = typeof a2uiSchema === "string" ? JSON.parse(a2uiSchema) : a2uiSchema;
			if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) catalogId = parsed.catalogId;
		} catch {}
		return [void 0, catalogId];
	}
	const contextEntries = agUi.context ?? [];
	for (const entry of contextEntries) {
		...
		return [value, match ? match[1] : void 0];
	}
}
/**
...
*/
const DEFAULT_GENERATION_GUIDELINES = `\
...
* Ported verbatim from the legacy `copilotkit.a2ui` defaults (OSS-248).
*/
const DEFAULT_DESIGN_GUIDELINES = `\
...
*/
function resolveA2UIToolParams(params) {
	return {
		model: params.model,
...
exports.DEFAULT_DESIGN_GUIDELINES = DEFAULT_DESIGN_GUIDELINES;
exports.DEFAULT_GENERATION_GUIDELINES = DEFAULT_GENERATION_GUIDELINES;
...
exports.resolveA2UICatalog = resolveA2UICatalog;
exports.resolveA2UIToolParams = resolveA2UIToolParams;
exports.runA2UIGenerationWithRecovery = runA2UIGenerationWithRecovery;
exports.splitA2UISchemaContext = splitA2UISchemaContext;
