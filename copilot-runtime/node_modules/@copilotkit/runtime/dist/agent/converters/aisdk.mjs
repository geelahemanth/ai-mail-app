...
*/
async function* convertAISDKStream(fullStream, abortSignal, pendingInterrupts) {
		...
		for await (const part of fullStream) {
			...
			switch (p.type) {
				...
				case "tool-approval-request": {
					const nested = p.toolCall ?? {};
					const toolCallId = p.toolCallId ?? nested.toolCallId;
					if (!toolCallId) throw new Error("AI SDK tool-approval-request is missing toolCallId");
					const state = ensureToolCallState(toolCallId);
					...
					if (!state.hasArgsDelta && nested.input !== void 0) {
						let serialized = "";
						try {
							serialized = typeof nested.input === "string" ? nested.input : JSON.stringify(nested.input);
						} catch {
					...
					pendingInterrupts?.push({
						...
						message: toolName ? `Approve "${toolName}"?` : void 0,
