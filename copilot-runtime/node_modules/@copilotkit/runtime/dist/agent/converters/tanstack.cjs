...
*/
async function* convertTanStackStream(stream, abortSignal, pendingInterrupts) {
	...
	for await (const chunk of stream) {
		...
		if (type === "CUSTOM" && raw.name === "approval-requested") {
			...
			if (toolCallId) pendingInterrupts?.push({
				id: toolCallId,
				toolCallId,
				reason: "tool_approval",
				message: value.toolName ? `Approve "${value.toolName}"?` : void 0,
