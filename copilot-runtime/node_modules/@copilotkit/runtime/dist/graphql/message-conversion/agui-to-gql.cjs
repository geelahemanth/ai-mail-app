...
//#region src/graphql/message-conversion/agui-to-gql.ts
...
function normalizeMessageContent(content) {
	if (typeof content === "string" || typeof content === "undefined") return content || "";
	if (Array.isArray(content)) return content.map((part) => {
		if (part?.type === "text") return part.text;
		if (part?.type === "binary") return part.data || part.url || part.filename || `[binary:${part.mimeType}]`;
		return "";
	}).filter(Boolean).join("\n");
	if (content && typeof content === "object") try {
...
function aguiToGQL(messages, actions, coAgentStateRenders) {
	...
	for (const message of messages) {
		if (isAgentStateMessage(message)) {
			...
			gqlMessages.push(new require_index.AgentStateMessage({
				id: message.id,
				agentName,
				state,
				role: require_index.Role.assistant
			}));
...
function aguiTextMessageToGQLMessage(message) {
	...
	let roleValue;
	if (message.role === "developer") roleValue = require_index.Role.developer;
	else if (message.role === "system") roleValue = require_index.Role.system;
	else if (message.role === "assistant") roleValue = require_index.Role.assistant;
	else roleValue = require_index.Role.user;
	return new require_index.TextMessage({
		id: message.id,
...
function aguiToolMessageToGQLResultMessage(message, toolCallNames) {
	...
	return new require_index.ResultMessage({
		id: message.id,
...
function aguiMessageWithImageToGQLMessage(message) {
	...
	let roleValue;
	if (message.role === "assistant") roleValue = require_index.Role.assistant;
	else roleValue = require_index.Role.user;
	...
	return new require_index.ImageMessage({
		id: message.id,
