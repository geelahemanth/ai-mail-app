...
function configureAgentForRequest(params) {
	...
	if (require_runtime$1.isA2UIEnabled(runtime.a2ui) || a2uiEnabledByCatalog) {
		const { agents: targetAgents, enabled: _enabled, injectA2UITool, ...a2uiOptions } = runtime.a2ui ?? {};
		if ((!targetAgents || targetAgents.includes(agentId)) && typeof agent.use === "function") agent.use(new _ag_ui_a2ui_middleware.A2UIMiddleware({
			...a2uiOptions,
...
async function parseRunRequest(request) {
	try {
		...
		_copilotkit_shared.logger.error("Invalid run request body:", error);
...
async function parseConnectRequest(request) {
	try {
		...
		_copilotkit_shared.logger.error("Invalid connect request body:", error);
		return new Response(JSON.stringify({
			error: "Invalid request body",
			details: error instanceof Error ? error.message : String(error)
		}), {
