...
import { EndpointType } from "./types.mjs";
import { extractParametersFromSchema } from "./mcp-tools-utils.mjs";
import { BuiltInAgent } from "../../agent/index.mjs";
import telemetryClient from "../telemetry-client.mjs";
import { CopilotKitMisuseError, getZodParameters, isTelemetryDisabled, readBody } from "@copilotkit/shared";
...
var CopilotRuntime = class {
	...
	get instance() {
		if (!this._instance) this._instance = new CopilotRuntime$1(this.runtimeArgs);
	...
	handleServiceAdapter(serviceAdapter) {
		this.runtimeArgs.agents = Promise.resolve(this.runtimeArgs.agents ?? {}).then(async (agents) => {
			...
			if (isAgentsListEmpty) {
				...
				if (languageModel) agentsList.default = new BuiltInAgent({ model: languageModel });
				else if (serviceAdapter.provider && serviceAdapter.model) agentsList.default = new BuiltInAgent({ model: `${serviceAdapter.provider}/${serviceAdapter.model}` });
			...
			if (actions) {
				...
				agentsList = this.assignToolsToAgents(agentsList, [...this.getToolsFromActions(actions), ...mcpTools]);
			}
			return agentsList;
		});
	}
	...
	createOnBeforeRequestHandler(params) {
		return async (hookParams) => {
			...
			if (params?.middleware?.onBeforeRequest) {
				...
				const { inputMessages, outputMessages } = aguiToGQL(body.messages).reduce((acc, msg) => {
	...
	*/
	...
	async getToolsFromMCP(options) {
		...
		if (!createMCPClient) throw new CopilotKitMisuseError({ message: "MCP Integration Error: `mcpServers` were provided, but the `createMCPClient` function was not passed to the CopilotRuntime constructor. Please provide an implementation for `createMCPClient`." });
		...
		for (const config of effectiveEndpoints) {
			...
			try {
				...
				const toolDefs = Object.entries(toolsMap).map(([toolName, tool]) => {
					const zodSchema = getZodParameters(extractParametersFromSchema(tool));
					return {
						name: toolName,
...
function resolveEndpointType(endpoint) {
	if (!endpoint.type) if ("deploymentUrl" in endpoint && "agents" in endpoint) return EndpointType.LangGraphPlatform;
	else return EndpointType.CopilotKit;
	return endpoint.type;
}

//#endregion
export { CopilotRuntime, copilotKitEndpoint, langGraphPlatformEndpoint, resolveEndpointType };
//# sourceMappingURL=copilot-runtime.mjs.map